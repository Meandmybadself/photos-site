...
<body>
${bodyContent}
<script>document.addEventListener('keydown',function(e){if(e.key==='ArrowLeft'){var a=document.querySelector('.photo-nav-prev');if(a)a.click()}if(e.key==='ArrowRight'){var a=document.querySelector('.photo-nav-next');if(a)a.click()}})</script>
</body>
</html>
`;
}
...
${gridCards}
  ...
  for (let i = 0; i < singles.length; i++) {
    ...
    const next = i < singles.length - 1 ? singles[i + 1] : null;
    await generateSinglePhotoPage(singles[i], prev, next);
  }
...
async function generateSinglePhotoPage(photo, prev, next) {
  const photoDir = join(PATHS.photos, photo.id);
  await mkdir(photoDir, { recursive: true });

  const navPrev = prev
    ? `<a class="photo-nav photo-nav-prev" href="/photos/${esc(prev.id)}/">&larr; Previous</a>`
    : '<span class="photo-nav"></span>';
  const navNext = next
    ? `<a class="photo-nav photo-nav-next" href="/photos/${esc(next.id)}/">Next &rarr;</a>`
    : '<span class="photo-nav"></span>';
