...
function groupCard(group) {
  return `  <a class="group-card" href="/albums/${esc(group.slug)}/">
    ...
    <div class="group-card-info">
      ...
      <p>${group.photoCount} photo${group.photoCount !== 1 ? 's' : ''}</p>
    </div>
  </a>`;
}
...
function photoThumb(slug, photo) {
  return `  <a class="photo-thumb" href="/albums/${esc(slug)}/${esc(photo.id)}/">
    <img src="${esc(photo.thumbnail)}" alt="${esc(photo.caption)}" loading="lazy" width="600">
  </a>`;
}

export async function generateSite(groupsIndex, manifests) {
  // Homepage and /albums/ (same content)
  const homepageBody = `<header>
  <h1><a href="/">Photos</a></h1>
</header>
<main class="group-grid">
${groupsIndex.groups.map(g => groupCard(g)).join('\n')}
</main>`;
...
@media (max-width: 640px) {
  ...
  .photo-grid { grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 0.5rem; }
